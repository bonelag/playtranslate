...
// PlayTranslate: kept upstream's 4. Tested 6 on Snapdragon 8 Gen 2 — got
// ~10-30% slower on Thor, suggesting threads spilled onto the LITTLE A510
// cores and dragged down the big-cluster throughput. 4 is the sweet spot for
// the 1×X3 + 4×A715 cluster.
...
// PlayTranslate: greedy decoding for deterministic translation output.
// Upstream default was 0.3f (chat-tuned). For translation we want max-likelihood
// selection so retries on the same prompt produce the same output.
...
static llama_context *init_context(llama_model *model, const int n_ctx = DEFAULT_CONTEXT_SIZE) {
    ...
    // PlayTranslate: Flash Attention forced off 2026-05-14. Auto-on correlated
    // with degenerate "8888..." Q4_0 output on vivo V2507A (Dimensity 9400+,
    // Android 16). Paired with the armv9-variant block in
    // third-party/llama.cpp/ggml/src/CMakeLists.txt — these are co-suspects
    // covering the same SVE2-correlated failure mode. If we ever isolate the
    // bug to one specific path, re-enable the other. See
    // memory/project_sme_armv9_2_bug.md.
