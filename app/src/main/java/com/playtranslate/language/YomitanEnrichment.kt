...
class YomitanEnrichment(
    ...
    suspend fun applyTo(
        ...
        val imported = failOpen<ImportedTerms?>(null, "term lookup") {
            lookupImportedTerms(word, reading, fallbackForms)
        } ?: return packResponse
     ...
     *  lemma): the word as passed first, then the engine's fallback forms. */
    private suspend fun lookupImportedTerms(
        ...
        val direct = YomitanDataStore.termSensesFor(appContext, sourceLang, word, reading)
