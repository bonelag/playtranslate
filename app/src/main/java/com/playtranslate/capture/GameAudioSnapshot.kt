 ...
 * [com.playtranslate.ui.SentenceAnkiContentFragment] deletes it in
 * onDestroyView. A selection's locator pins the exact file, so a later card
 * can never clobber the audio an earlier card trimmed and approved — the
 * whole family of "wrong/missing audio after snapshot churn" races is
 * structurally impossible rather than guarded against at every consumer.
 * The mtime baked into selection keys remains as cache-key identity and as
 * a fail-closed backstop for the residual cases (OS cache purge, orphan
 * sweep of a >6 h zombie sheet).
 *
